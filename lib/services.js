...
exports.home = function (request, reply) {
  ...
  posts.feed(true, function (err, postItems) {
    if (err) {
...
exports.search = function (request, reply) {
  ...
  if (request.payload && request.payload.keyword) {
    search.find(request.payload.keyword, function (err, posts) {
      if (err) {
        return reply(Boom.wrap(new Error('No posts available'), 400));
      }

      ctx.posts = posts;
      ctx.keyword = request.payload.keyword;

      reply.view('search', ctx);
    });
  } else {
    ctx.posts = [];
